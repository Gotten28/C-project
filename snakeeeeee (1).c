...
// Gaston & Jenna -- Spawns the head of the snake in the center.
void setSegments(int i, int x, int y) //Jenna & Gaston
{
  snakeSegment[i].x = COLS / 2 + x;
  snakeSegment[i].y = LINES / 2 + y;
}

// Gaston & Jenna -- Spawns the head of the snake in the center, then the rest of the snake body to its left.
void SnakeSpawn()
{
    srand(time(0));
    dir = rand() %4;
...
void handleInput(int movement) //Gaston & Jenna
{
  ...
  //if snake hits wall -Jason/Alice
